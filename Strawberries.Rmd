...
##Come back to this part right here
```{r}
...
```{r}
strwb_survey_mkt <- strwb_survey_mkt %>% 
  select(-temp23, -temp43, -temp44, -temp45, -cv)
```
...
colnames(strwb_survey_chem)[colnames(strwb_survey_chem) == "chem_total"] <- "Chem_Num"
...
Questions for EDA 
Where are most strawberries grown and what markets do they go to?
-need to know the states
-what market they go to 

```{r}

```
What pesticides are used and how does it vary by state?
```{r}

```
What state uses the most harmful pesticides?
-what does each harm level mean 
-need to by state 
-eventually group by harm level 
...
ggplot(plot3_data, aes(x = reorder(State, -Value), y = Value)) + 
  geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "States", y = "Sales",
       title = "Organic Strawberry Sales ($) in 2021")
